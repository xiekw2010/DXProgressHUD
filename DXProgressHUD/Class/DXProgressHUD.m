...
#define iOS7Before ([[[UIDevice currentDevice] systemVersion] compare:@"7.0" options:NSNumericSearch] == NSOrderedAscending)


#import "DXProgressHUD.h"
#import <QuartzCore/QuartzCore.h>

NSString * const DXProgressHUDDidReceiveTouchEventNotification = @"DXProgressHUDDidReceiveTouchEventNotification";
NSString * const DXProgressHUDWillDisappearNotification = @"DXProgressHUDWillDisappearNotification";
NSString * const DXProgressHUDDidDisappearNotification = @"DXProgressHUDDidDisappearNotification";
NSString * const DXProgressHUDWillAppearNotification = @"DXProgressHUDWillAppearNotification";
NSString * const DXProgressHUDDidAppearNotification = @"DXProgressHUDDidAppearNotification";

NSString * const DXProgressHUDStatusUserInfoKey = @"DXProgressHUDStatusUserInfoKey";
...
@interface DXProgressHUD ()

@property (nonatomic, readwrite) DXProgressHUDMaskType maskType;
...
@property (nonatomic, assign) DXProgressTranslucentStyle style;
@property (nonatomic, strong) UIColor *ringLayerColor;


- (void)showProgress:(float)progress
              status:(NSString*)string
            maskType:(DXProgressHUDMaskType)hudMaskType;
...
- (void)setStyle:(DXProgressTranslucentStyle)style
{
    _style = style;
    [self _updateStyleRelatedViews];
}

- (void)_updateStyleRelatedViews
{
    switch (self.style) {
        case DXProgressTranslucentStyleBlack:
        {
            self.stringLabel.textColor = [UIColor whiteColor];
            if (iOS7Before) {
                _hudView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.8];
            }else {
                UIToolbar *hudView = (UIToolbar *)_hudView;
                hudView.barStyle = UIBarStyleBlack;
            }
            self.ringLayerColor = [UIColor whiteColor];
        }
            break;
        case DXProgressTranslucentStyleWhite:
        {
            self.stringLabel.textColor = [UIColor darkGrayColor];
            if (iOS7Before) {
                _hudView.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.8];
            }else {
                UIToolbar *hudView = (UIToolbar *)_hudView;
                hudView.barStyle = UIBarStyleDefault;
            }
            self.indefiniteAnimatedView.color = [UIColor blackColor];
            self.ringLayerColor = [UIColor darkGrayColor];
        }
            break;
    }
}

+ (DXProgressHUD*)sharedView {
    static dispatch_once_t once;
    static DXProgressHUD *sharedView;
...
+ (void)show {
    [[self sharedView] showProgress:-1 status:nil maskType:DXProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:DXProgressHUDMaskTypeNone];
}
...
+ (void)showProgress:(float)progress {
    [[self sharedView] showProgress:progress status:nil maskType:DXProgressHUDMaskTypeNone];
}

+ (void)showProgress:(float)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:DXProgressHUDMaskTypeNone];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[DXProgressHUD sharedView] displayDurationForString:string];
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        if ([[UIImage class] instancesRespondToSelector:@selector(imageWithRenderingMode:)]) {
          SVProgressHUDSuccessImage = [[UIImage imageNamed:@"DXProgressHUD.bundle/success"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
          SVProgressHUDErrorImage = [[UIImage imageNamed:@"DXProgressHUD.bundle/error"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        } else {
          SVProgressHUDSuccessImage = [UIImage imageNamed:@"DXProgressHUD.bundle/success"];
          SVProgressHUDErrorImage = [UIImage imageNamed:@"DXProgressHUD.bundle/error"];
        }
        SVProgressHUDRingThickness = 4;
        self.style = DXProgressTranslucentStyleBlack;
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case DXProgressHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case DXProgressHUDMaskTypeGradient: {
...
- (NSDictionary *)notificationUserInfo
{
    return (self.stringLabel.text ? @{DXProgressHUDStatusUserInfoKey : self.stringLabel.text} : nil);
}
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent *)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:DXProgressHUDDidReceiveTouchEventNotification object:event];
}

#pragma mark - Master show/dismiss methods

- (void)showProgress:(float)progress status:(NSString*)string maskType:(DXProgressHUDMaskType)hudMaskType {
    ...
    if(self.maskType != DXProgressHUDMaskTypeNone) {
    ...
    if(self.alpha != 1) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DXProgressHUDWillAppearNotification
                                                            object:nil
                                                          userInfo:userInfo];
        ...
        [UIView animateWithDuration:0.15
                         ...
                         completion:^(BOOL finished){
                             [[NSNotificationCenter defaultCenter] postNotificationName:DXProgressHUDDidAppearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration {
    ...
    if ([self.imageView respondsToSelector:@selector(setTintColor:)]) {
      self.imageView.tintColor = self.ringLayerColor;
    } else {
      image = [self image:image withTintColor:self.ringLayerColor];
    }
...
- (void)dismiss {
    NSDictionary *userInfo = [self notificationUserInfo];
    [[NSNotificationCenter defaultCenter] postNotificationName:DXProgressHUDWillDisappearNotification
                                                        object:nil
                                                      userInfo:userInfo];
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0 || self.hudView.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:DXProgressHUDDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (UIActivityIndicatorView *)indefiniteAnimatedView {
    if (_indefiniteAnimatedView == nil) {
        _indefiniteAnimatedView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectZero];
        _indefiniteAnimatedView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhiteLarge;
    }
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        ...
        _backgroundRingLayer = [self createRingLayerWithCenter:center
                                                        radius:SVProgressHUDRingRadius
                                                     lineWidth:SVProgressHUDRingThickness
                                                         color:[self.ringLayerColor colorWithAlphaComponent:0.1]];
...
- (BOOL)isClear { // used for iOS 7
    return (self.maskType == DXProgressHUDMaskTypeClear || self.maskType == DXProgressHUDMaskTypeNone);
}
...
- (UIView *)hudView {
    if(!_hudView) {
        if (iOS7Before) {
            ...
            hudView.barStyle = self.style == DXProgressTranslucentStyleWhite ? UIBarStyleDefault : UIBarStyleBlack;
            _hudView = hudView;
        }
